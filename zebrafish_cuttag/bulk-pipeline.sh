...
$macs2bin/macs2 callpeak -t $bam_file -g 1.5e9 -f BAMPE -n $base_file --outdir $outdir -q 0.01 -B --SPMR --keep-dup all 2> $logdir/"$base_file".macs2.narrow
# macs2 broad peak calling
>&2 echo "[info] macs2 broad peak calling"
$macs2bin/macs2 callpeak -t $bam_file -g 1.5e9 -f BAMPE -n $base_file --outdir $outdirbroad --broad --broad-cutoff 0.1 -B --SPMR --keep-dup all 2> $logdir/"$base_file".macs2.broad
# macs2 broad peak calling
...
$macs2bin/macs2 callpeak -t $bam_file -g 1.5e9 -f BAMPE -n $base_file --outdir $outdirseac -q 0.01 -B --SPMR --keep-dup all 2> $logdir/"$base_file".seacr
...
cat $peak | grep -v -e "chrM" | $bedopsbin/sort-bed - > blacklist_filtered/$peak
cat $summit | grep -v -e "chrM" | $bedopsbin/sort-bed - > blacklist_filtered/$summit
...
if [ ! -d $dir ] || [ ! -f $dir/$peak ] ; then
    cat $workdir/$dir/"$peak_filename" | grep -v -e "chrM" | $bedopsbin/sort-bed - > $workdir/$dir/$peak
fi
>&2 echo "[info] Getting Fasta sequences"
